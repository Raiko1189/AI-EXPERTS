...
class Client:
    ...
    def request(
        ...
        *,
        ...
        if headers is None:
            headers = {}
         # Fix: ensure OAuth token is refreshed if it is missing, expired,
         # or not an instance of OAuth2Token (e.g., when it is a dict).
         # Previously, dict tokens were not refreshed, causing the
         # Authorization header to be missing in API requests.
        if api:
